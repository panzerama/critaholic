...
class HomePageTest(TestCase):
    # TODO later on, change the home page to a menu system and move this to
    # '/init'
...
class InitViewTest(TestCase):
    ...
    def test_displays_all_monsters_in_order(self):
        ...
        Initiative.objects.create(
            creature_name="beholder",
            initiative_value=10,
            hit_points=150,
            encounter=correct_encounter_,
        )
        Initiative.objects.create(
            creature_name="displacer beast",
            initiative_value=11,
            hit_points=100,
            encounter=correct_encounter_,
        )
        ...
        Initiative.objects.create(
            ...
            initiative_value=20,
            hit_points=250,
            encounter=incorrect_encounter_,
        )
        Initiative.objects.create(
            creature_name="Falkrainne",
            initiative_value=1,
            hit_points=2,
            encounter=incorrect_encounter_,
        )
    ...
    def test_init_name_validation_errors_displayed_on_init_view(self):
        encounter_ = Encounter.objects.create()
        response = self.client.post(
            "/init/%d/" % encounter_.id,
            data={"init_name": "", "init_num": 18, "init_hp": 150},
        )
